...
class Graph {
  public:
    ...
    void add_directed_edge(size_t u, size_t v, int w = 1) { d_e.at(u).emplace_back(v, w); }
    void add_undirected_edge(size_t u, size_t v, int w = 1) {
        d_e.at(u).emplace_back(v, w);
        d_e.at(v).emplace_back(u, w);
    }
    const std::vector<Edge>& operator[](const size_t& index) { return d_e.at(index); }
    size_t size() { return d_size; }
    void reverse() {
        std::vector<std::vector<Edge>> tmp(d_size + 1);
        for (size_t x = 0; x <= d_size; ++x)
            for (const auto& [y, weight] : d_e.at(x))
